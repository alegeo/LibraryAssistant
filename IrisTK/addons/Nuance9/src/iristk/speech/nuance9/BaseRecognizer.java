...
public class BaseRecognizer extends Thread {
	...
	* Recognizer parameters
	...
	public BaseRecognizer() throws NuanceException {
		File configFile = new File(IrisUtils.getPackagePath("Nuance9"), "config.xml");
		File tempConfigDir = IrisUtils.getTempDir("Nuance9");
	...
	public void startRecognize() throws NuanceException {
		result = null;
		recRecognizerStart();
	...
	public void stopRecognize() {
		if (recognizerThread != null && recognizerThread.running) {
			...
			try {
				...
				while (cont) {
					...
					if (status.getValue() == SWIrec.SWIrec_STATUS_SUCCESS) {
						result = new NuanceResult(resultRef.getValue(), makeWords);
						break OUTER;
					} else if (status.getValue() == SWIrec.SWIrec_STATUS_NO_MATCH) {
						result = new NuanceResult(ResultType.FINAL, RecognitionResult.NOMATCH);
						break OUTER;
					} else if (status.getValue() == SWIrec.SWIrec_STATUS_STOPPED) {
						result = new NuanceResult(ResultType.CANCELED);
						break OUTER;	
					} else if (status.getValue() == SWIrec.SWIrec_STATUS_MAX_SPEECH) {
						result = new NuanceResult(ResultType.MAXSPEECH);
						break OUTER;	
					}
				}
			} catch (NuanceException e) {
